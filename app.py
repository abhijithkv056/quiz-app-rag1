...
# --- Main Content ---
# Checking if the user has provided API keys
missing_openai = not openai_api_key or "sk-" not in openai_api_key
missing_anthropic = not anthropic_api_key
if missing_openai and missing_anthropic and not az_openai_api_key:
    st.write("#")
    st.warning("Please set up your API keys in the environment variables to continue...")
else:
    # Sidebar
    with st.sidebar:
        st.divider()
        ...
        for model in MODELS:
            if "openai" in model and not missing_openai:
                ...
                models.append(model)
            elif "azure-openai" in model and az_openai_api_key:
                models.append(model)
